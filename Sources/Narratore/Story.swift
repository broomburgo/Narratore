...
extension SceneType {
  ...
  /// The unique id assigned to a `Scene`.
  ///
  /// It must be unique for each scene type, and it's used to decode a scene.
  ///
  /// It's value is `"\(Self.self)"`, simply representing the type of the scene.
  ///
  /// **PLEASE NOTE:** to guarantee its uniqueness, each scene must have a distrinct string description for the type,
  /// and to do that it's important that scenes are not defined in namespaces. Consider for example this case:
  ///
  /// ```swift
  /// enum Namespace1 {
  ///   struct SomeScene {}
  ...
  /// ```
  ///
  /// while `Namespace1.SomeScene` and `Namespace2.SomeScene` are distinct types in Swift, **their string description
  /// is the same**, that is, `"SomeScene"`, and this is not allowed in `Narratore`, so please avoid namespaces for scenes.
...
/// The `steps` computed property in a scene must return a linear sequence of `SceneStep`.
///
